...
import com.mycompany.logproject.config.KafkaProperties;
...
class LogprojectKafkaResourceIT {
    ...
    void setup() {
        ...
        LogprojectKafkaResource kafkaResource = new LogprojectKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/logproject-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/logproject-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
